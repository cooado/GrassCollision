...
namespace Artngame.INfiniDy {
	...
	public class InfiniGRASS_DemoRealTimev15 : MonoBehaviour {
				...
				if (!OnlySunWind) {
					...
					if (GrassManager.Grass_painting) {
						...
						for (int i=0; i<IconsGrass.Count; i++) {
							...
							//v1.5
							int break_at = 14;
							float grass_X = Grass_GUI_startX + 60 * i - 500;
							float grass_Y = 0;
							if (i > break_at) {
								grass_Y = Grass_GUI_startY + 40 -10;
								grass_X = Grass_GUI_startX + 60 * (i-break_at-1) - 500;
							}
							...
							if (GUI.Button (new Rect (grass_X, grass_Y, 60, sizeing), IconsGrass [i])) {	
								...
								if (i == 10 | i == 11) {
									...
									GrassManager.LOD_distance = 20;
									GrassManager.LOD_distance1 = 40;
									GrassManager.LOD_distance2 = 50;
					...
					//TINT			
					GUI.TextArea (new Rect (Grass_GUI_startX + 125 + 125, Grass_GUI_startY + 60 + 5 + 30 + 40 + 30 + 30 + 40 + 9 + 30 + 5, 110, 22), "Tint power:");
